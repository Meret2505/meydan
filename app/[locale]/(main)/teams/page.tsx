  ...
  return (
    <>
      ...
      <div className="px-6 pt-4 pb-24">
        ...
        {/* The two team queries stream in here so the title above paints
            immediately on navigation instead of blocking on the DB. */}
        <Suspense fallback={<TeamsSkeleton label={t("teams.my_teams")} />}>
          <TeamsList userId={userId} locale={locale} />
        </Suspense>
      </div>
    </>
  );
...
async function TeamsList({ userId, locale }: { userId: string; locale: string }) {
...
function TeamsSkeleton({ label }: { label: string }) {
  return (
    <>
      <div className="text-[13px] font-bold text-text-muted mt-5 mb-3">{label}</div>
      <div className="flex flex-col gap-3">
        <RowCardSkeleton />
        <RowCardSkeleton />
      </div>
